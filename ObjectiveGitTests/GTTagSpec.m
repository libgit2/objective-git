...
beforeEach(^{
	...
	tag = (GTTag *)[repository lookupObjectBySHA:tagSHA error:&error];
...
describe(@"+tagByCreatingTagNamed:target:message:tagger:force:inRepository:error:", ^{
	...
	beforeEach(^{
		...
		originalTag = [repository lookupObjectBySHA:originalTagSHA objectType:GTObjectTypeTag error:NULL];
	});
