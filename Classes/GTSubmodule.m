...
- (GTSubmoduleStatus)status:(NSError **)error {
	...
	if (gitError != GIT_OK) {
		if (error != NULL) *error = [GTError errorForGitError:gitError description:@"Failed to get submodule %@ status.", self.name];
...
- (BOOL)reload:(NSError **)error {
	...
	if (gitError != GIT_OK) {
		if (error != NULL) *error = [GTError errorForGitError:gitError description:@"Failed to reload submodule %@.", self.name];
		return NO;
	}
...
- (BOOL)sync:(NSError **)error {
	...
	if (gitError != GIT_OK) {
		if (error != NULL) *error = [GTError errorForGitError:gitError description:@"Failed to synchronize submodule %@.", self.name];
		return NO;
	}
...
- (GTRepository *)submoduleRepository:(NSError **)error {
	...
	if (gitError != GIT_OK) {
		if (error != NULL) *error = [GTError errorForGitError:gitError description:@"Failed to open submodule %@ repository.", self.name];
		return nil;
	}
...
- (BOOL)writeToParentConfigurationDestructively:(BOOL)overwrite error:(NSError **)error {
	...
	if (gitError != GIT_OK) {
		if (error != NULL) *error = [GTError errorForGitError:gitError description:@"Failed to initialize submodule %@.", self.name];
		return NO;
	}
