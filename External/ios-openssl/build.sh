...
# OpenSSL ${OPENSSL_VERSION} for iOS 7.0 that contains code for armv6, armv7, arm64 and i386.
...
build()
{
   ...
   perl -i -pe "s|^CC= gcc|CC= ${CLANG} -miphoneos-version-min=${SDK_VERSION} -arch ${ARCH}|g" Makefile
...
lipo \
	...
	"/tmp/openssl-arm64/lib/libcrypto.a" \
...
lipo \
	...
	"/tmp/openssl-arm64/lib/libssl.a" \
